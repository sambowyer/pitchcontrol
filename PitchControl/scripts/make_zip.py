...
def  main():
  ...
  installer = "\installer\PitchControl Installer.exe"
   
  if demo:
    installer = "\installer\PitchControl Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\PitchControl manual.pdf" 
  ]
  ...
  ZIPNAME = "PitchControl-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "PitchControl-v" + FULLVERSIONSTR + "-win-demo.zip"
